...
FLAGS['output_dir'] = ''
FLAGS['annotate_copy_slots'] = False
...
class Processor(object):
    ...
    def __init__(self, schemas, correct_categorical_state_file):
        self._schemas = schemas
        ...
        with open(correct_categorical_state_file, 'r') as f:
            for line in f:
    ...
    def _generate_dial_turns(self, turns, dial_id):
        ...
        for i in range(len(turns))[::2]:
            ...
            update_states = self._get_update_states(
                ...
                for slot, slot_value in slot_values_dict.items():
                    ...
                    if slot in service_schema.categorical_slots:
                        if (slot_value not in service_schema.get_categorical_slot_values(
                            ...
                            print('Value %s not contained in slot %s, dial_id %s, ',    # FIXME
...
class SGDBaselineDataPreprocessor(DataPreprocessor):
    def __init__(self, data_file, schema_file, correct_categorical_state_file):
        super(SGDBaselineDataPreprocessor, self).__init__(data_file)
        self.processor = Processor(Schema(schema_file), correct_categorical_state_file)
        # above code is for original implementation of SGDBaseline
        # below is for our own implementations
        self.slots = set()
        with open(schema_file, "r", encoding='utf-8') as f:
            schema = json.load(f)
            for domain_content in schema:
                for slot_content in domain_content['slots']:
                    self.slots.add(slot_content['name'])
        self.schema = {
            slot['name']:{
                'is_categorical': slot['is_categorical'],
                'possible_values': slot['possible_values']
            } for service in schema for slot in service['slots']
        }
    ...
    def create_dataset(self, filter_file, target_file):
        ...
        """
        raw_data = self.filter(filter_file)
        converted_dials = self.processor.convert_to_dstc(raw_data.keys(), raw_data)
        sgd_data = []
        for dialog in converted_dials:
            system_utt = ""
            state = {slot: "none" for slot in self.slots}
            for turn in dialog['turns']:
                if turn['speaker'] == "USER":
                    intent = None
                    requested_slots = []
                    user_utterance = turn['utterance'].lower()
                    turn_state = {slot: "none" for slot in self.slots}
                    for domain_frame in turn['frames']:
                        requested_slots += domain_frame['state']['requested_slots']
                        if domain_frame['state']['active_intent'] != 'NONE':
                            intent = domain_frame['state']['active_intent']
                        for slot, value_list in domain_frame['state']['slot_values'].items():
                            known_slot_flag = False
                            for value in value_list:
                                value_ = value.strip(string.punctuation)
                                if value == state[slot] or value_ == state[slot]:
                                    known_slot_flag = True
                                    break
                            if known_slot_flag:
                                continue
                            selected_value = "none"
                            for value in value_list:
                                striped_value = value.strip(string.punctuation)
                                if striped_value == 'dontcare':
                                    selected_value = 'dontcare'
                                    break
                                elif self.schema[slot]['is_categorical']:
                                    if striped_value in self.schema[slot]['possible_values']:
                                        selected_value = striped_value
                                        break
                                    if value in self.schema[slot]['possible_values']:
                                        selected_value = value
                                        break
                                else:
                                    if striped_value in user_utterance or striped_value in system_utt:
                                        selected_value = striped_value
                                        break
                                    elif value in user_utterance or value in system_utt:
                                        selected_value = value
                                        break
                            turn_state[slot] = selected_value
                            state[slot] = selected_value
                    sgd_data.append({
                    ...
                    system_utt = turn['utterance'].lower()
        with open(target_file, "w", encoding='utf-8') as f:
            json.dump(sgd_data, f, indent=2)


def main():
    data_processor = SGDBaselineDataPreprocessor("MultiWOZ2_1/data.json", \
         "schema.json", "correct_categorical_state_values.tsv")
    if not os.path.exists("MultiWOZ2_1_SGDBaseline"):
        os.mkdir("MultiWOZ2_1_SGDBaseline")
    data_processor.create_dataset("MultiWOZ2_1/trainListFile.txt", "MultiWOZ2_1_SGDBaseline/train.json")
    data_processor.create_dataset("MultiWOZ2_1/valListFile.txt", "MultiWOZ2_1_SGDBaseline/valid.json")
    data_processor.create_dataset("MultiWOZ2_1/testListFile.txt", "MultiWOZ2_1_SGDBaseline/test.json")

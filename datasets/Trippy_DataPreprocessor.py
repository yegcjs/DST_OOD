...
""" target dataset should be like
    json:{
        <dialog_id>: {
            ...
            "system_utterence": <system_utterance>,
            "history": <history>,
            "known_slots": [<domain-slot>, <domain-slot>, ..., <domain-slot>],
            "informed_slots": [<domain-slot>, <domain-slot>, ..., <domain-slot>],
            "labels": {
                <domain-slot>: [
                    "none"|"dontcare"|"copy"|"inform"|"reference"|"true"|"false", 
                    "none"|<copy_value>|<reference_slot>
                ]
                ...
            }
        }
...
def create_examples(input_data, acts, slot_list, label_maps,
                    set_type="default",
                    append_history=True,
                    ...
                    unk_token="[UNK]",
                    analyze=False):
    sys_inform_dict = load_acts(acts)
...
class TrippyDataPreprocessor(DataPreprocessor):
    def __init__(self, config_file, system_act_file, data_file):
        super(TrippyDataPreprocessor, self).__init__(data_file)
        with open(system_act_file, "r", encoding='utf-8') as f:
            self.system_acts = json.load(f)
        with open(config_file, "r", encoding='utf-8') as f:
            config = json.load(f)
        self.all_slots = [slot.lower() for slot in config['slots']]
        self.value_map = config['label_maps']

    def create_dataset(self, filter_file, target_file):
        raw_data = self.filter(filter_file)
        examples = create_examples(raw_data, self.system_acts, self.all_slots, self.value_map)
        dataset = []
        for example in tqdm(examples):
            ...
            for slot in self.all_slots:
                label = example.class_label[slot]
                info = 'none'
                if label == "copy_value":
                    start = example.text_a_label[slot].index(1)
                    if 0 in example.text_a_label[slot][start:]:
                        end = start + example.text_a_label[slot][start:].index(0)
                    else:
                        end = len(example.text_a_label[slot])
                    info = " ".join(example.text_a[start:end])
                elif label == "refer":
                    info = example.refer_label[slot]
                labels[slot] = [label, info]
            dataset.append({
                "user_utterance": " ".join(example.text_a),
                "system_utterance": " ".join(example.text_b),
                "history": " ".join(example.history),
                "known_slots": copy.deepcopy(known_states),
                "informed_slots": copy.deepcopy([slot for slot, value in example.inform_label.items() if value != 'none']),
                "labels": copy.deepcopy(labels),
                "state": copy.deepcopy(example.values)
            })
            known_states = [slot for slot, value in example.values.items() if value != 'none']
...
def main():
    src_base = "MultiWOZ_OoD"
    target_base = "MultiWOZ_OoD_Trippy"

    DataInit.init_trippy(src_base, target_base)
    # data_processor = TrippyDataPreprocessor("MultiWOZ2_1/multiwoz21config.json", "MultiWOZ2_1/system_acts.json",
    #                                         "MultiWOZ2_1/data.json")
    # if not os.path.exists("MultiWOZ2_1_Trippy"):
    #     os.mkdir("MultiWOZ2_1_Trippy")
    # data_processor.create_dataset("MultiWOZ2_1/trainListFile.txt", "MultiWOZ2_1_Trippy/train.json")
    # data_processor.create_dataset("MultiWOZ2_1/valListFile.txt", "MultiWOZ2_1_Trippy/valid.json")
    # data_processor.create_dataset("MultiWOZ2_1/testListFile.txt", "MultiWOZ2_1_Trippy/test.json")

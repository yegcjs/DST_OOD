...
def read_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, args, max_line = None):
...
def get_seq(pairs, lang, mem_lang, batch_size, type, sequicity, args):
    if pairs is None:
        return None
    # args = vars(args)
...
def dump_pretrained_emb(word2index, index2word, dump_path):
    ...
    for i in tqdm(range(len(word2index.keys()))):
        ...
        E.append(e)
    if not os.path.exists("/".join(dump_path.split('/')[:-1])):
        os.mkdir("/".join(dump_path.split('/')[:-1]))
...
def prepare_data_seq(training, file_train, file_dev, file_test, args, task="dst", sequicity=0, batch_size=10):
    args = vars(args)
    ...
    ontology = json.load(open("datasets/auxfiles/ontology.json", 'r'))
    ...
    if training:
        pair_train, train_max_len, slot_train = read_langs(file_train, gating_dict, ALL_SLOTS, "train", lang, mem_lang, sequicity, training, args)
        train = get_seq(pair_train, lang, mem_lang, batch_size, True, sequicity, args)
        nb_train_vocab = lang.n_words
        pair_dev, dev_max_len, slot_dev = read_langs(file_dev, gating_dict, ALL_SLOTS, "dev", lang, mem_lang, sequicity, training, args)
        dev   = get_seq(pair_dev, lang, mem_lang, eval_batch, False, sequicity, args)
        pair_test, test_max_len, slot_test = read_langs(file_test, gating_dict, ALL_SLOTS, "test", lang, mem_lang, sequicity, training, args)
        test  = get_seq(pair_test, lang, mem_lang, eval_batch, False, sequicity, args)
        ...
        emb_dump_path = f'{folder_name}/data/emb{len(lang.index2word)}.json'
    ...
    test_4d = []
    if args['except_domain']!="":
        pair_test_4d, _, _ = read_langs(file_test, gating_dict, ALL_SLOTS, "dev", lang, mem_lang, sequicity, training, args)
        test_4d  = get_seq(pair_test_4d, lang, mem_lang, eval_batch, False, sequicity, args)

    max_word = max(train_max_len, dev_max_len, test_max_len) + 1
    if pair_train is not None:
        print("Read %s pairs train" % len(pair_train))
    if pair_dev is not None:

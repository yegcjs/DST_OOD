...
from ..trade_utils.measures import wer, moses_multi_bleu
from ..trade_utils.masked_cross_entropy import *
from ..trade_utils.config import *
import pprint


class TRADE(nn.Module):
    def __init__(self, hidden_size, lang, path, task, lr, dropout, slots, gating_dict, args, nb_train_vocab=0):
        ...
        self.encoder = EncoderRNN(self.lang.n_words, hidden_size, self.dropout, self.args)
        self.decoder = Generator(self.lang, self.encoder.embedding, self.lang.n_words, hidden_size, self.dropout,
                                 self.slots, self.nb_gate, self.args)
    ...
    def print_loss(self):
        ...
        self.print_every += 1
        return print_loss_avg, 'L:{:.2f},LP:{:.2f},LG:{:.2f}'.format(print_loss_avg, print_loss_ptr, print_loss_gate)
    ...
    def evaluate(self, dev, slot_temp, early_stop=None, last_only=False):
        ...
        for j, data_dev in pbar:
            ...
            for bi in range(batch_size):
                ...
                if set(data_dev["turn_belief"][bi]) != set(predict_belief_bsz_ptr) and self.args["genSample"]:
                    print("True", set(data_dev["turn_belief"][bi]))
                    print("Pred", set(predict_belief_bsz_ptr), "\n")

        if self.args["genSample"]:
    ...
    def evaluate_metrics(self, all_prediction, from_which, slot_temp, last_only=False):
        # pdb.set_trace()
        total, turn_acc, turn_slot_acc, joint_acc, F1_pred, F1_count = 0, 0, 0, 0, 0, 0
        for d, v in all_prediction.items():
            for t in range(len(v)):
                if last_only and t != len(v)-1:
                    continue
                cv = v[t]
                if set(cv["turn_belief"]) == set(cv[from_which]):
                    joint_acc += 1

                oracle_prev_state = set() if t == 0 else set(v[t-1]['turn_belief'])
                pred_prev_state = set() if t == 0 else set(v[t-1][from_which])
                if (set(cv["turn_belief"]) - oracle_prev_state) == (set(cv[from_which]) - pred_prev_state):
                    turn_acc += 1
                ...
                turn_slot_acc += temp_acc   # XXX: original ,
...
class EncoderRNN(nn.Module):
    def __init__(self, vocab_size, hidden_size, dropout, args, n_layers=1):
        ...
        if self.args["load_embedding"]:
            with open(f"{args['path']}/data/emb{vocab_size}.json") as f:
                E = json.load(f)
...
class Generator(nn.Module):
    def __init__(self, lang, shared_emb, vocab_size, hidden_size, dropout, slots, nb_gate, args):
